...
GAMEDIR="/${directory#/}/ports/spacegrunts"
java_runtime="zulu17.54.21-ca-jre17.0.13-linux"
jar_filename="spacegrunts.dat"

cd "$GAMEDIR" || { pm_message "Space Grunts: game folder missing."; pm_finish; exit 1; }
...
fail() {
  pm_message "Space Grunts: $* See spacegrunts/log.txt."
  sleep 5
  cleanup
  exit 1
}
...
[ -f "$GAMEDIR/$jar_filename" ] || fail "Copy your owned spacegrunts.dat to spacegrunts/spacegrunts.dat."
...
"$JAVA_HOME/bin/java" -Xmx64m -cp runtime/spacegrunts-host.jar org.portmaster.spacegrunts.VerifyGame "$GAMEDIR/$jar_filename" || fail "Unsupported or damaged game JAR. Check the README checksum."
source "$GAMEDIR/display.inc" || fail "Display helper missing."
spacegrunts_display_setup || fail "Use auto or WIDTHxHEIGHT in resolution.txt."
printf 'Firmware: %s; display: %s\n' "$CFW_NAME" "$spacegrunts_display_description"
export SDL_GAMECONTROLLERCONFIG="$sdl_controllerconfig"
export HOTKEY=back
$GPTOKEYB2 java -c "$GAMEDIR/spacegrunts.ini" &
mapper_pid=$!
...
$ESUDO env "${display_env[@]}" "$weston_dir/westonwrap.sh" headless noop kiosk crusty_glx_gl4es \
  ...
  "-Dspacegrunts.dat=$GAMEDIR/$jar_filename" "-Dspacegrunts.saves=$SAVEDIR" \
  -Dspacegrunts.fullscreen=true "${display_java[@]}" \
  -cp "$GAMEDIR/runtime/spacegrunts-host.jar:$GAMEDIR/$jar_filename" org.portmaster.spacegrunts.Main
