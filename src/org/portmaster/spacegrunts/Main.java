...
import com.orangepixel.spacegrunts.myCanvas;
...
public class Main extends myCanvas {
    ...
    public static Lwjgl3ApplicationConfiguration configuration() throws Exception {
        int width = Integer.getInteger("spacegrunts.width",640), height = Integer.getInteger("spacegrunts.height",480);
        ...
        Path saves = Paths.get(System.getProperty("spacegrunts.saves","saves")).toAbsolutePath();
        java.nio.file.Files.createDirectories(saves);
        Lwjgl3ApplicationConfiguration cfg = new Lwjgl3ApplicationConfiguration();
        cfg.setTitle("Space Grunts"); cfg.setWindowedMode(width,height);
        ...
        cfg.disableAudio(Boolean.getBoolean("spacegrunts.noAudio"));
        cfg.setInitialVisible(!Boolean.getBoolean("spacegrunts.hidden"));
        if (Boolean.getBoolean("spacegrunts.fullscreen")) cfg.setFullscreenMode(Lwjgl3ApplicationConfiguration.getDisplayMode());
        return cfg;
    }

    public static void main(String[] args) throws Exception {
        VerifyGame.check(Paths.get(System.getProperty("spacegrunts.dat","spacegrunts.dat")));
        System.out.println("Space Grunts PortMaster host 0.1.0 | " + System.getProperty("os.arch") + " | 60 updates/s");
    ...
    @Override public void create() {
        ...
        myPreferences = new com.orangepixel.spacegrunts.DesktopPreferences() {
            @Override public Preferences getPreferences(String name, boolean forceCreation) {
                Path path = Paths.get(System.getProperty("spacegrunts.saves","saves"),"legacy-preferences",name);
                if (!forceCreation && !java.nio.file.Files.isRegularFile(path)) return null;
        ...
        System.out.println("GAME_CREATE_OK offline; PortMaster keyboard input; saves=" + System.getProperty("spacegrunts.saves","saves"));
    }
    ...
    @Override public void render() {
        ...
        super.render();
        frames++;
        int smoke = Integer.getInteger("spacegrunts.smokeFrames",0);
        if (smoke > 0 && frames >= smoke) {
            String capture = System.getProperty("spacegrunts.capture");
    ...
    @Override public void pause() { saveProfile(); super.pause(); }
    ...
    private void saveProfile() {
        try {
            java.lang.reflect.Field field = myCanvas.class.getDeclaredField("activePlayer");
            field.setAccessible(true);
            if (field.get(null) != null) myCanvas.saveSettings();
